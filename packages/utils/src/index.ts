...
function findMonorepoPackageJsonPath() {
  ...
  while (path.dirname(currentDir) !== currentDir) {
    ...
    const packageFile = readPackageUp.sync({
      cwd: currentDir,
      normalize: true,
    })
...
export function hasDependency(depName: string, version = '*') {
  ...
  // en existe un)
  const packageFile = readPackageUp.sync({ normalize: true })
  if (
  ...
  if (monorepoPackageJsonPath) {
    const monoRepoPackageFile = readPackageUp.sync({
